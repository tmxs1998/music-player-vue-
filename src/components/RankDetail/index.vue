<template>
  <div class="rankDetail">
    ...
    <!-- 排行列表 -->
    <div class="rankList" ref="wrapper">
      <div class="content">
        <ul>
...
import { getRankList, getSongUrlByMid } from 'api/api.js'
...
export default {
  ...
  methods: {
    ...
    back () {
      this.$router.go(-1)  // 返回排行列表页
    },
    openPlay (index) {
      this.addSongList(this.songlist) // 点击歌名显示音乐播放器
    ...
    handleList (list) {
      ...
      result = list.map((item, index) => {
        let {albummid, albumname, singer, songmid, songname} = item.data // 从数据中剥取需要的数据并保存
        // 拼接图片地址
  ...
  async created () { // 将created生命周期变为异步
    let  { rankid } = this.$route.params // 获取路径参数中的歌手id+
    let songdata = []
    await getRankList(rankid).then((res) => {
      this.rankname = res.topinfo.ListName
      this.avator =  res.topinfo.pic
      if (res.songlist.length > 99){
        songdata = res.songlist.slice(0,99)
      } else {
        songdata = res
      }
    })
    let { result, mids } = this.handleList(songdata) // 将获得的列表数据再加工
    ...
    this.songlist = finalData // 更新list数组为获取到的数组
    this.$nextTick(() =>{
